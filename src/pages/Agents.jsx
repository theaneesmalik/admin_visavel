...
const Agents = () => {
  ...
  const { data, loading } = useFetch('/users/?role=agent')
  useEffect(() => {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((item) => {
      const companyMatch = item.company?.toLowerCase().includes(query.toLowerCase())
  ...
  const handleShow = (index) => {
    ...
    navigate('/agents/show', { state: user })
  }
  const handleStatus = (index) => {
    const agent = filteredRows[index]
    const requestOptions = {
      method: 'put',
      url: '/users',
    ...
    axios
      ...
      .then((result) => {
        if (result.data) {
          const rowIndex = rows.findIndex((row) => row._id === agent._id)
  ...
  return (
    <div className='centerTable'>
      ...
      {openDialog && (
        <MyDialog
          title='Confirm'
          des={`Are you sure you want to delete Agent ${del.name} profile?`}
          actions={[
            {
              onClick: () => {
                handleDel(del.id)
